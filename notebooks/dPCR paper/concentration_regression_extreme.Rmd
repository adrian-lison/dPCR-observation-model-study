...
sim_data <- mapply(function(alpha_true) {
  alpha_true <- log(alpha_true)
  beta_true = 0.2
...
if (rerun_models) {
  ...
  saveRDS(res_beta_all, here::here("data", "results", "regression_extreme_simulated_beta.rds"))
} else {
  res_beta_all <- readRDS(here::here("data", "results", "regression_extreme_simulated_beta.rds"))
}
```
...
if (rerun_models) {
  ...
  saveRDS(res_alpha_all, here::here("data", "results", "regression_extreme_simulated_alpha.rds"))
} else {
  res_alpha_all <- readRDS(here::here("data", "results", "regression_extreme_simulated_alpha.rds"))
}
```
...
res_beta_all |> 
  ...
  scale_x_continuous(labels = scales::label_percent(accuracy = 0.01), breaks = seq(0.9991, 1, by = 0.0003)) +
...
ggsave(here::here("figures", "dPCR paper", "regression_extreme_validation_beta.pdf"), width = 10, height = 6)
```
```{r}
...
res_alpha_all |> 
  ...
  geom_line(data = data.frame(x = ftrans(log(seq(1e+05*4, 1e+05*8, length.out=100))), y = seq(1e+05*4, 1e+05*8, length.out=100)), aes(x=x,y=y), linetype = "dashed") +
  facet_wrap(~model_facet, ) +
...
ggsave(here::here("figures", "dPCR paper", "regression_extreme_validation_alpha.pdf"), width = 10, height = 6)
```
