...
This notebooks examines differences in dPCR-based concentrations estimates obtained from two technical replicates either obtained by empirical averaging (i.e. arithmetic mean of concentrations) or by computing the ML estimate of the concentration from the combined partition counts. The example data used here is based on digital droplet PCR (ddPCR).
...
Here we see that there is no additional bias resulting from the choice of weighting, but the possible deviations are larger than the in the previous section. Below we show deviations for differences in the total number of partitions below 2000 (which is the case for the majority of measurements).
```{r fig.width = 10, fig.height = 5, echo = FALSE}
avg_sim |> 
  slice_sample(prop = 1, replace = F) |> 
...
Overall, these results show that the ML estimate provides more accurate results than the averaging approach. The plot below compares the absolute error of the concentration estimates based on the averaging (blue) and ML (red) approach as a function of the concentration. The ribbons shows the 95% interval of absolute errors, while the lines show the median absolute error (MAE). According to the MAE, the ML estimate is slightly more accurate, however this difference is rather small compared to the variance of errors. To summarize, using the ML estimate instead of averaging concentrations gives slightly better estimates, but it is not going to change the bigger picture unless the total number of partitions varies considerably between replicates.
