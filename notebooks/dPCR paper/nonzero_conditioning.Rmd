...
conditional_df <- data.frame(lambda = seq(0.01, 14, 0.01)) |> 
  mutate(
    p_nondetect = sapply(lambda, est_nondetection_prob_pre, m_partitions = n, c = c, cv = example_cv, n_replicates = n_replicates, noise_dist = "lnorm"),
    mean_unconditional = lambda,
    cv_unconditional = sapply(lambda, est_cv_pre_lnorm, m_partitions = n, c = c, cv = example_cv, n_replicates = n_replicates),
