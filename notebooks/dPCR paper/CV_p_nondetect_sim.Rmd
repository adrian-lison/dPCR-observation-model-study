...
```{r}
compute_dat_cv_pre <- function(example_n_replicates, example_c, example_m_partitions, example_cv_values, example_lambda_values, simulate = FALSE, seed = 0) {
  set.seed(seed)
...
ggplot(aes(x=lambda)) + 
  geom_line(aes(y = conc_cv_sim_gamma, color = m_partitions, group = m_partitions), alpha = 0.5) + 
  geom_line(aes(y  = conc_cv_gamma, color = m_partitions, group = m_partitions))) |> 
  format_cv_plot() +
...
```{r}
compute_nondetection_probs <- function(example_lambda_values, example_c, example_m_partitions, example_cv, noise_dist, example_replicates, seed = 0) {
set.seed(seed)
